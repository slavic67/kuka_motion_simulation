...
with open('lab2_output.txt',"r",encoding='utf-8') as file:
    file.readline()
    for line in file:
...
with open('lab2_input.txt',mode='r',encoding='utf-8') as file:
    for line in file:
        vx,vy,om=line.split()
        om=float(om)
        vx=float(vx)
...
#
# #4d
dX_vost=numpy.array(x_id_array)-numpy.array(x_vost)
dY_vost=numpy.array(y_id_array)-numpy.array(y_vost)
dPSI_vost=numpy.array(dpsi_id_array)-numpy.array(psi_vost)
