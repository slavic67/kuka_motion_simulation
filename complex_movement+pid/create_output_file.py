...
with open('lab3_output.txt',"r",encoding='utf-8') as file:
    file.readline()
    for line in file:
...
with open('lab3_input.txt',mode='r',encoding='utf-8') as file:
    for line in file:
        vx,vy,om=line.split()
        om=float(om)
        vx=float(vx)
...
for i in range(0,t*10):
    Vx_id_temp.append(Vx_id_array[i]*np.cos(psi_id_array[i])-Vy_id_array[i]*np.sin(psi_id_array[i]))
    Vy_id_temp.append(Vx_id_array[i]*np.sin(psi_id_array[i])+Vy_id_array[i]*np.cos(psi_id_array[i]))


for i in range(1,t*10):
    x_id_array.append(x_id_array[i - 1] + dt * Vx_id_temp[i-1])
    y_id_array.append(y_id_array[i - 1] + dt * Vy_id_temp[i-1])
...
for i in range(0,t*10):
    Vx_real_nsk.append(Vx_real_array[i]*np.cos(dpsi_real_array[i])-Vy_real_array[i]*np.sin(dpsi_real_array[i]))
    Vy_real_nsk.append(Vx_real_array[i]*np.sin(dpsi_real_array[i])+Vy_real_array[i]*np.cos(dpsi_real_array[i]))
...
for i in range(0,t*10):
    Vx_id_nsk.append(Vx_id_array[i]*np.cos(Psi_id_nsk[i])-Vy_id_array[i]*np.sin(Psi_id_nsk[i]))
    Vy_id_nsk.append(Vx_id_array[i]*np.sin(Psi_id_nsk[i])+Vy_id_array[i]*np.cos(Psi_id_nsk[i]))

X_id_nsk=[0]
Y_id_nsk=[0]
for i in range(1,t*10):
    X_id_nsk.append(X_id_nsk[i-1]+dt*Vx_id_nsk[i-1])
    Y_id_nsk.append(Y_id_nsk[i-1] + dt * Vy_id_nsk[i-1])
...
dX_ip_nsk=np.array(X_real_nsk)[0:110]-np.array(X_id_nsk)[0:110]
dY_ip_nsk=np.array(Y_real_nsk)[0:110]-np.array(Y_id_nsk)[0:110]
dPSI_ip_nsk=np.array(Psi_id_nsk)[0:110]-np.array(Psi_real_nsk)[0:110]
...
# ССК
# разность идеальных и реальных координат
dX_ip=np.array(x_real_array)[0:110]-np.array(x_id_array)[0:110]
dY_ip=np.array(y_real_array)[0:110]-np.array(y_id_array)[0:110]
dPSI_ip=np.array(psi_id_array)[0:110]-np.array(dpsi_real_array)[0:110]

#разность идеальных и восстановленных координат
dX_ios=np.array(x_id_array)[0:110]-np.array(Xp_nsk)[0:110]
dY_ios=np.array(x_id_array)[0:110]-np.array(Yp_nsk)[0:110]
DPSI_ios=np.array(psi_id_array)[0:110]-np.array(Psip_nsk)[0:110]
...
with open('lab6_output.txt',"r",encoding='utf-8') as file:
    file.readline()
    for line in file:
...
# ax1.set_xlabel('X,m')
# ax1.set_ylabel('Y,m')
# ax1.plot(X_id_nsk,Yp_nsk)
# ax1.plot(X_real_nsk,Y_real_nsk)
# ax1.plot(Xp_nsk,Yp_nsk)
